...
R muss zuerst heruntergeladen und dann installiert werden. Unter den sogennannten `r r6objTableAbbrev$add_abbrev(psAbbrev = "CRAN", psMeaning = "Comprehensive R Archive Network")`-Mirror Webseiten ist R als ausführbares Programm für die Betriebssystem `Windows`, `Mac OSX` und `Linux` erhältlich.

```{r ScreenShotCRAN, conv.odg=TRUE, odg.path="odg", odg.graph.cache=TRUE, odg.out.format=c("pdf","png"), png.path="png", fig.align='center', echo=FALSE, results='asis'}
knitr::include_graphics(path = "png/ScreenShotCRAN.png")
```


Zusätzlich zu R empfielt es sich, die Entwicklungsumgebung RStudio (https://www.rstudio.com/) zu verwenden.

```{r ScreenShotRstudio, conv.odg=TRUE, odg.path="odg", odg.graph.cache=TRUE, odg.out.format=c("pdf","png"), fig.align='center', echo=FALSE, results='asis'}
knitr::include_graphics(path = "png/ScreenShotRstudio.png")
```
...
```{r vonNeumannComputerArch, conv.odg=TRUE, odg.path="odg", odg.graph.cache=TRUE, odg.out.format=c("pdf","png"), fig.align='center', echo=FALSE, results='asis'}
knitr::include_graphics(path = "png/vonNeumannComputerArch.png")
```
...
```{r Arbeitsspeicher1, conv.odg=TRUE, odg.path="odg", odg.graph.cache=TRUE, odg.out.format=c("pdf","png"), fig.align='center', echo=FALSE, results='asis', out.width="5cm"}
knitr::include_graphics(path = "png/Arbeitsspeicher1.png")
```
...
```{r Arbeitsspeicher2, conv.odg=TRUE, odg.path="odg", odg.graph.cache=TRUE, odg.out.format=c("pdf","png"), fig.align='center', echo=FALSE, results='asis', out.width="5cm"}
knitr::include_graphics(path = "png/Arbeitsspeicher2.png")
```
...
```{r Arbeitsspeicher3, conv.odg=TRUE, odg.path="odg", odg.graph.cache=TRUE, odg.out.format=c("pdf","png"), fig.align='center', echo=FALSE, results='asis', out.width="5cm"}
knitr::include_graphics(path = "png/Arbeitsspeicher3.png")
```
...
```{r ArbeitsspeicherZugriff, conv.odg=TRUE, odg.path="odg", odg.graph.cache=TRUE, odg.out.format=c("pdf","png"), fig.align='center', echo=FALSE, results='asis', out.width="5cm"}
knitr::include_graphics(path = "png/ArbeitsspeicherZugriff.png")
```
...
## Einlesen von Daten
Eine Verwendungsart von R ist die statistische Analyse von Daten. Zu diesem Zweck müssen wir die Daten zuerst einlesen. Erst dann können wir sie analysieren. Die wichtigste Funktion um Daten in R einzulesen, heisst `read.table()`. Mit dieser Funktion können Daten aus Files, welche Tabellen-artig organisiert sind, eingelesen werden. Haben die einzulesenden Daten ein spezifischers Format, so wie zum Beispiel `r r6objTableAbbrev$add_abbrev(psAbbrev = "CSV", psMeaning = "Comma Separated Values")`, so gibt es spezialisierte Funktionen, wie `read.csv2()`, welche Daten im CSV-Format einlesen können. Ein Beipiel dafür sei nachfolgend gezeigt. Die Option `file = "csv/br_gew.csv"` gibt an, wo im Dateisystem das File mit den Daten zu finden ist.
...
```{r LinModAbline, fig.show=TRUE, out.width="12cm", fig.align="center"}
